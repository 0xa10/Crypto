...
import random
import time, sys
from hmac_sha1_timing_attack import time_verify, verify_hmac, avg

ATTACK_TIMER = 5
SENSITIVITY = 2
PICKUP_THRESHOLD = ((ATTACK_TIMER - SENSITIVITY)/1000.0) * 2

def chr_word(word):
    return chr((word >> 8) & 0xff) + chr(word & 0xff)
...
def main(argv):
    ...
    while len(signature) < 20:
        ...
        #for c in random.shuffle(range(256)): # random optimization?
            iterations += 1
            sig_test = (signature + chr_word(c)).ljust(20,"\x00")
            current_time = time_verify(target_msg, sig_test.encode("hex"))
            print "Guess %s took %f seconds, baseline of %f, difference of %dms" % (chr_word(c).encode("hex"), current_time, prev_time, (current_time-prev_time)*1000)
            max_guess = max(max_guess, (c, (current_time-prev_time)), key = lambda x : x[1])
            if current_time > prev_time + PICKUP_THRESHOLD:
                # verify correctness
                print "Found candidate %s, verifying..." % (chr_word(c).encode("hex"))
                ...
                if not (avg_time > prev_time + PICKUP_THRESHOLD):
                    ...
                    continue
                print "Confirmed hash byte %s, realigning" % (chr_word(c).encode("hex"))
                signature += chr_word(c)
            ...
            c = max_guess[0]
            print "Failed to find current hash byte, using %s" % (chr_word(c).encode("hex"))
            signature += chr_word(c)
            guessed = True
